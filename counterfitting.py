...
def compute_vsp_pairs(word_vectors, vocabulary, rho=0.2):
	...
	"""
	print "Pre-computing all the word pairs which are to be enforced via the VSP term."
	print "Warning: use of the VSP term increases experiment time (~1h)."
	
	vocabulary = list(vocabulary)
	vsp_pairs = {}
	t_initial = time.time()

	for idx, word_i in enumerate(vocabulary):
		for j in range(idx + 1, len(vocabulary)):

			word_j = vocabulary[j]
			original_distance = distance(word_vectors[word_i], word_vectors[word_j])

			if original_distance <= rho:
				vsp_pairs[(word_i, word_j)] = original_distance 
				vsp_pairs[(word_j, word_j)] = original_distance

		if idx % 500 == 0: 
			print "Progress:", round((float(idx) / len(vocabulary)) * 100, 3), "percent." \
				  "# Pairs:", len(vsp_pairs), "Time:", round(time.time() - t_initial, 1), "seconds."
...
def one_step_SGD(word_vectors, synonym_pairs, antonym_pairs, vsp_pairs, current_experiment, iteration_count=0):
	...
	if current_experiment.hyper_k3 > 0:
		...
		for (word_i, word_j) in vsp_pairs:
			...
			if original_distance < new_distance and vsp_pairs[(word_i, word_j)] < 0.2: 
